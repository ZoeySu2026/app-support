...
APPS["poste"] = {
    "name": "Posté",
    "short": "Posté",
    "tagline": "把照片變成一塊玻璃藝術：17 種玻璃、8 種印刷工藝、6 種紙材，全部在手機上完成",
    "faq": [
        ("照片會上傳嗎？",
         "<p>不會。所有玻璃、印刷、紙材的運算都在你的手機上完成。沒有帳號、沒有伺服器。</p>"),
        ("怎麼疊第二層玻璃？",
         "<p>在玻璃頁按「＋」加一層。每一層可以選不同玻璃、不同遮罩範圍（圓形、方形、橫帶、自動天空、自動主體）。"
         "點縮圖切換要調的那一層。</p>"),
        ("光怎麼調？",
         "<p>在畫面上直接拖光點；或選「光」參數用光盤。真機上把手機傾斜，光會跟著流動。</p>"),
        ("印刷的墨色與紙色在哪裡？",
         "<p>底色面板 → 印刷，選了任一種印刷後，下方會出現「墨量、墨色、紙色、網點、錯版、質感」。「重設」回到預設。</p>"),
        ("怎麼存成限動貼紙？",
         "<p>輸出頁 → 匯出 → 存成貼紙。存的是透明背景 PNG，在 Instagram 限時動態用貼圖工具貼上。</p>"),
        ("角標的地名或日期不對",
         "<p>角標從照片的拍攝資訊自動帶入。在輸出頁點文字欄位直接改，或清空。</p>"),
        ("護照是什麼？",
         "<p>每次以郵票、票根或日期郵票格式存進相簿，就會蓋一枚到護照，依地點疊放。長按郵票可以刪除。</p>"),
        ("影片匯出很慢",
         "<p>匯出時間與影片長度、解析度、玻璃層數成正比。建議 60 秒內、1080p。匯出過程請不要切到背景。</p>"),
        ("怎麼刪除草稿或作品？",
         "<p>起始頁「我的作品」長按縮圖 → 刪除。刪除 App 會清空所有草稿、作品與護照。</p>"),
        ("要付費嗎？",
         "<p>目前版本全部功能免費，沒有 App 內購買、沒有訂閱、沒有廣告。</p>"),
    ],
    "privacy_extra": [
        ("相簿",
         "<p>讀取<strong>你選擇的</strong>照片與影片來製作作品；把成品、貼紙與影片存回相簿。只有你選的檔案會被讀取，"
         "不會掃描整個相簿。</p>"),
        ("照片中的日期與地點（EXIF）",
         "<p>用來自動填寫作品角標上的地名、日期與時間。座標會透過 <strong>Apple 的地理編碼服務</strong>換成地名，"
         "這是 iOS 系統功能；Posté 沒有自己的伺服器，不會保存或傳送座標。你可以在編輯器內改掉或清空這些文字。</p>"),
        ("動作感測器",
         "<p>用手機傾斜來改變玻璃的光線方向。資料只在裝置上即時使用，不儲存。</p>"),
        ("存在裝置上的資料",
         "<p>草稿、作品、護照郵票與偏好設定都存在 App 的沙盒內。刪除 App 就會一併刪除；也可以在 App 內逐一刪除。</p>"),
        ("付費",
         "<p>目前版本完全免費，沒有 App 內購買、沒有訂閱、沒有廣告。未來若加入付費功能，購買由 Apple App Store 處理，"
         "我們仍不會取得你的付款資料。</p>"),
    ],
}
